...
void EventLoop::add_timer(std::chrono::milliseconds interval,
  ...
  if (timer_fd_ == -1) {
    throw std::runtime_error(std::format("timerfd_create failed: {}",
                                         std::strerror(errno)));
  }
  ...
  if (timerfd_settime(timer_fd_, 0, &spec, nullptr) == -1) {
    close(timer_fd_);
    timer_fd_ = -1;
    throw std::runtime_error(std::format("timerfd_settime failed: {}",
                                         std::strerror(errno)));
  }
