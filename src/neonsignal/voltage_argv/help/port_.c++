...
std::string help::port_() const {
  ...
  if (mode() == Mode::redirect) {
    return std::format(
        ...
        "  {}\n\n"
        "{}\n"
        "  HTTP port for redirector service (1-65535).\n\n"
        "{}\n"
        "  {}\n\n"
        "{}\n"
        "  9090 (non-privileged HTTP port)\n\n"
        "{}\n"
        "  {} spin --port=80\n"
        "  {} spin --port=9090\n",
        ...
        ansi("REDIRECT_PORT").bright_cyan().str(), ansi("DEFAULT").stylish().bold().str(),
  ...
  return std::format(
      ...
      "  {}\n\n"
      "{}\n"
      "  HTTPS listen port for server (1-65535).\n\n"
      "{}\n"
      "  {}\n\n"
      "{}\n"
