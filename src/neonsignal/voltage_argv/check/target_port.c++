...
unsigned long long check::target_port() const {
  ...
  if (std::holds_alternative<unsigned long long>(this->value_)) {
    ...
    const char *env_port = std::getenv("REDIRECT_TARGET_PORT");
    if (env_port != nullptr) {
      port_num = std::stoull(env_port);
    } else {
      port_num = 9443; // Default HTTPS port
    }
  } else {
    throw std::invalid_argument(
        std::format("invalid argument type for --target-port, expected number"));
  }

  if (port_num == 0 || port_num > 65535) {
    throw std::out_of_range(
        std::format("--target-port {} is out of valid range (1-65535)", port_num));
  }
