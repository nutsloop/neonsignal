...
std::string check::mail_cookie_name() const {
  std::string value;
  ...
  if (std::holds_alternative<std::string>(this->value_)) {
    ...
    const char *env_value = std::getenv("NEONSIGNAL_MAIL_COOKIE_NAME");
    ...
    throw std::invalid_argument(
        std::format("invalid argument type for --mail-cookie-name, expected string"));
  }
