...
std::string check::mail_command() const {
  std::string value;
  ...
  if (std::holds_alternative<std::string>(this->value_)) {
    ...
    const char *env_value = std::getenv("NEONSIGNAL_MAIL_COMMAND");
    ...
    throw std::invalid_argument(
        std::format("invalid argument type for --mail-command, expected string"));
  }
