...
std::string check::mail_domains() const {
  std::string value;
  ...
  if (std::holds_alternative<std::string>(this->value_)) {
    ...
    const char *env_value = std::getenv("NEONSIGNAL_MAIL_DOMAINS");
    ...
    throw std::invalid_argument(
        std::format("invalid argument type for --mail-domains, expected string"));
  }
