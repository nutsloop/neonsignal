...
std::string check::mail_url_hits() const {
  std::string value;
  ...
  if (std::holds_alternative<std::string>(this->value_)) {
    ...
    const char *env_value = std::getenv("NEONSIGNAL_MAIL_URL_HITS");
    ...
    throw std::invalid_argument(
        std::format("invalid argument type for --mail-url-hits, expected string"));
  }
