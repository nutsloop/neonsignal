...
bool check::mail_save_db() const {
  if (std::holds_alternative<std::string>(this->value_)) {
    return parse_bool(std::get<std::string>(this->value_), "--mail-save-db");
  }
  if (std::holds_alternative<std::nullptr_t>(this->value_)) {
    const char *env_value = std::getenv("NEONSIGNAL_MAIL_SAVE_DB");
    if (env_value != nullptr) {
      return parse_bool(env_value, "NEONSIGNAL_MAIL_SAVE_DB");
    }
    return true;
  }

  throw std::invalid_argument(
      std::format("invalid argument type for --mail-save-db, expected boolean"));
}
