...
bool ApiHandler::cpu_stream(const std::shared_ptr<Http2Connection> &conn, std::uint32_t stream_id,
  ...
  conn->last_cpu_time_ns = static_cast<std::uint64_t>(ts.tv_sec) * 1000000000ull +
  ...
  std::cerr << "HEADERS on fd=" << conn->fd << " stream=" << stream_id << " path=" << path
            << " method=" << method << " authority=" << authority << " (cpu sse)\n";
  return true;
}
