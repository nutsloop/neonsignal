...
bool ApiHandler::stats(const std::shared_ptr<Http2Connection> &conn, std::uint32_t stream_id,
                       const std::string &path, const std::string &method,
                       const std::string &authority) {
  std::string body = "{\"files_served\":" + std::to_string(served_files_.load()) +
  ...
  std::cerr << "HEADERS on fd=" << conn->fd << " stream=" << stream_id << " path=" << path
            << " method=" << method << " authority=" << authority << " (api)\n";
  return true;
}
