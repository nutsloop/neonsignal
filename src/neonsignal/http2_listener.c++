...
Http2Listener::Http2Listener(EventLoop &loop, ThreadPool &pool, SSL_CTX *ssl_ctx,
    ...
    : loop_(loop), pool_(pool), ssl_ctx_(ssl_ctx), config_(std::move(config)), router_(router),
      served_files_(served_files), page_views_(page_views), event_clients_(event_clients),
      ...
      db_(std::make_unique<Database>(config_.db_path)), auth_(config_.rp_id, config_.origin, *db_),
      api_handler_(std::make_unique<ApiHandler>(loop_, auth_, router_, *db_, served_files_,
                                                page_views_, event_clients_, redirect_service_ok_)),
