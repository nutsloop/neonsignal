...
struct TypeForm {
  std::variant<PrimitiveEnergy, SequenceType, ClusterType, WaveType, NamedEnergy, MaybeEnergy,
               ResultEnergy, ReferenceEnergy>
      essence;
...
struct VariantPattern {
  DataPath path;
  std::variant<std::monostate,                        // unit variant
...
struct Pattern {
  std::variant<WildcardVoid, HandleBind, NumberGlow, TextBeam, TruthValue, ClusterPattern,
               EntityPattern, VariantPattern, OrPattern, ReferencePattern>
      form;
...
struct Expression {
  std::variant<
      ...
      NumberGlow, FloatWave, TextBeam, GlyphAtom, TruthValue, SequenceLiteral, GridLiteral,
      // Operations - The Transformations
      BinaryBlast, UnaryTransform, CallBlast, MethodInvoke, FieldReach, IndexDive, MorphCast,
      QuantumTry, PipeFlow,
      // Control Flow - Navigate the Grid
      PortalBranch, ArcadeMatch, BlockPortal, LoopForever, LoopWhile, ChaseLoop, BounceBack,
      BailOut, CruiseOn,
      // Async - Turbo Mode
      RideWave,
      // Other Constructs
      QuantumFlip, EntitySpawn, ClusterBuild, PathResolve>
      essence;
...
struct Statement {
  std::variant<WireStatement, ExpressionPulse> form;
...
struct EntityDeclaration {
  ...
  Handle entity_name;
  std::vector<EnergyParameter> generic_params;
  std::variant<std::vector<EntitySlot>,                // record entity { x: T, y: U }
...
struct PlugDeclaration {
  ...
  std::variant<DataPath,             // plug grid::database
...
struct ModuleItem {
  std::variant<std::unique_ptr<PlugDeclaration>, std::unique_ptr<BeamDeclaration>,
               std::unique_ptr<WaveDeclaration>, std::unique_ptr<EntityDeclaration>,
               std::unique_ptr<VariantDeclaration>, std::unique_ptr<PowersDeclaration>,
               std::unique_ptr<ProtocolDeclaration>, std::unique_ptr<StaticDeclaration>,
