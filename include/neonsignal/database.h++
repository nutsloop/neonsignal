...
public:
  ...
  bool set_user_credential(std::uint64_t user_id, std::span<const std::uint8_t> credential_id,
  ...
  // Verification tokens
  bool store_verification(std::span<const std::uint8_t> token_hash, std::uint64_t user_id,
  ...
  // Session management
  std::string create_session(std::uint64_t user_id, std::string_view user, std::string_view state,
                             std::chrono::seconds ttl);
  ...
  bool upgrade_session_state(std::string_view session_id, std::string_view new_state,
  ...
  std::optional<CodexRun> create_codex_run(std::string_view brief_id, std::string_view cmdline);
  bool update_codex_run(const CodexRun &run);
  std::optional<CodexRun> fetch_codex_run(std::string_view run_id);
  bool store_codex_run_streams(std::string_view run_id, std::span<const std::uint8_t> stdout_bytes,
