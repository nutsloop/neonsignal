...
**Frontend Runtime and Applications:**
...
- ``neonsignal_redirect`` - HTTP to HTTPS redirect service
...
For CLI options and environment variables, see :ref:`neoncli`.

Start the Redirect Service
~~~~~~~~~~~~~~~~~~~~~~~~~~

The redirect service handles HTTP to HTTPS redirects and ACME challenges:

.. code-block:: bash

   # Run on port 9090 (firewall redirects 80 -> 9090)
   ./build/src/neonsignal_redirect spin
...
.. list-table::
   ...
   * - ``NEONSIGNAL_PORT``
     - ``9443``
     - HTTPS listen port
   * - ``NEONSIGNAL_WEBAUTHN_DOMAIN``
     - *none*
     - WebAuthn Relying Party ID
   * - ``NEONSIGNAL_WEBAUTHN_ORIGIN``
     - *none*
     - WebAuthn origin URL
   * - ``NEONSIGNAL_DB_PATH``
     - ``data/neonsignal.mdb``
     - LIBMDBX database file path
   * - ``NEONSIGNAL_WWW_ROOT``
     - ``public``
     - Static files root directory
   * - ``NEONSIGNAL_CERTS_ROOT``
     - ``certs``
     - TLS certificates root directory
...
.. code-block:: bash
   ...
   NEONSIGNAL_THREADS=3 ./build/src/neonsignal spin
...
.. list-table::
   ...
   * - ``REDIRECT_INSTANCES``
     - ``2``
     - Number of redirect service instances
   * - ``REDIRECT_HOST``
     - ``0.0.0.0``
     - Bind address
   * - ``REDIRECT_PORT``
...
.. code-block:: ini
   ...
   ExecStart=/usr/local/bin/neonsignal --systemd
...
.. code-block:: ini
   ...
   ExecStart=/usr/local/bin/neonsignal_redirect --systemd
...
For production deployment with Let's Encrypt certificates, see the :doc:`Deployment Guide <part4-operations/deployment>`.
...
- Learn about :doc:`Virtual Hosting <part2-architecture/virtual-hosting>` for multi-domain setups
- Set up :doc:`Let's Encrypt certificates <part4-operations/deployment>` for production
- Explore :doc:`SSE streaming <part3-features/sse>` for real-time features
